...
| Explorer browser UI: Events (eclipses, Moon phases, equinoxes, solstices, close approaches) | unstarted | the engine (`skyfix-almanac::{events,eclipses}`) and the CLI (`skyfix events`, `phases`, `seasons`) are complete and validated (`docs/ACCURACY.md` sections 9 and 12); eclipses have no CLI command yet either; only the browser screen itself is missing |
