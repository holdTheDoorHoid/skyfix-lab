...
| `meteor_showers(year, observer)` speed | unstarted | 0.2 s natively (one night per shower, 32 nights); computing a shower's night only when the view asks for it would make the year view instant **polish2:** 150–190 ms in WebAssembly under Node for a year at a place (`web/scripts/core-bench.mjs`; ACCURACY section 22) |
...
| Core-module size | completed | The package adds 191 KB raw and 77 KB gzipped on main 3f4fe4e, taking the module to 2 997 407 / 1 231 646 bytes, 2.6 KB under the 3 MB raw budget (mostly code: E5, the searches, the MPC parser, the serialisation of eleven calls). `opt-level = "z"` (the polish agent's lever) or a lazily loaded second module would restore headroom **polish2:** with the deep-time series and built at `opt-level = "z"` the module is 2 549 965 bytes raw, 1 152 112 gzipped (ACCURACY section 22) |
...
| Views that still write a literal "UTC" beside times that may be UT | completed | `events/`, `almanac/`, `navigate/`, `panel/selected.ts`, `panel/place.ts` (`formatOffset(…)` without the instant: "UTC−5:00:40" in 585 BC): use `scaleLabel(jd)` and `formatOffset(ms, jd)` **polish2:** the rest (the charts' captions and tables, Navigate's time fields and tables, Learn's simulator) name the clock's scale; `web/test/next/polish-strings.test.ts` fails on a "(UTC)", "in UTC" or "hover for UTC" in any view's source |
| The panel's zone reason before 1850 | completed | `panel/place.ts` says "From the place: America/New_York…" while the clock is local mean time; `lmtReason` and `zoneTooltip` give the words **polish2:** checked: before 1850 the panel gives `lmtReason` (navigate2) |
| "1990–2060" and "1990 to 2060" in the interface | completed | `navigate/session-panel.ts:140`, `events/lists.ts:137, 263, 424, 425`, `events/eclipses.ts:749, 758`, `almanac/almanac.ts:557` (the date input's range), `panel/when.ts:82`; `tierNotice`, `sightsOnlyText` and `outsideCoverage` name the real bounds **polish2:** the session panel's help names the validated years from the engine; the Events lists say their own engine's span (`listCoverage`); the source scan fails on a literal 1990 or 2060 |
| Navigate's sight-time field and expanded years | completed | `navigate/parse.ts` `UTC_PATTERN` takes four-digit years only; a `-0584-…` time gets a format error instead of "Sights are offered only between 1550 and 2650" **polish2:** checked: `UTC_PATTERN` takes signed years (`navigate-sightextras.test.ts`), and a time in 585 BC gets "Sights are offered only between 1550 and 2650" |
| The CLI's help text | completed | `crates/skyfix-cli/src/cli.rs:193` says "from 1990-01-01 to 2060-12-31" **polish2:** checked: the help takes the span from the engine (`validated_span()`, cli3) |
...
| Aiming the Sky view | completed | "Show in Sky" (Milky Way) and "See it up close" (the Moon) open the Sky view at the moment; pointing it at the galactic centre and the Moon's close-up inset are the sky2 agent's (`// sky2:`) **polish2:** closed: sky2 aims the dome and opens the Moon's close-up (its row "Aiming the Sky view from the Selected card") |
...
| The core module's size budget | completed | The lunar limb adds 45.3 KB raw / 20.7 KB gzipped to the core module; main alone (ab8f55c) is 2 997 407 bytes, 2.6 KB under the 3 MB budget, so the module is 42.7 KB over it (and 2.3 KB over 1.25 MB gzipped). Options: raise the budget, `opt-level = "z"` (about 5 %), or load the limb engine as a second module with its pack **polish2:** the budget was raised to 3 MB / 1.25 MB and the module is built at `opt-level = "z"`: 2 549 965 bytes raw, 1 152 112 gzipped (ACCURACY section 22) |
...
| The sky's darkness (Bortle class) kept between visits | completed | kept while the page is open (per explorer); a setting in `state.ts` would keep it, shared with the Sky view's magnitude limit when sky2 adds one **polish2:** a stored setting shared with the Sky view and the Events view |
...
| `web/scripts/ui-check.mjs` header | completed | the union merge of charts2 and time-ui left a duplicated line ("than on a real screen);") and two `ONLY=` usage lines in the header comment **polish2:** the header lists the fifteen blocks in run order with their ONLY names; the blocks run in that order |
...
## Expansion programme — integration polish (polish2 agent, 2026-09-25)

| item | status | notes |
|---|---|---|
| What a date in the labelled tier shows (the polish walk at 28 May 585 BC in Philadelphia and 21 June 2999 in Tromsø, every view and tab) | known | Each view says what it cannot show in one line, in plain words (`rangeWords`), with the covered years. The engines behind these views still answer only the validated tier or their own range. The Sky view draws no stars (`skyfix-starfield`; see the deeptime row above). Tonight's deep-sky ranking, meteor showers and extinction use `Sky::new()` or the star field. The Selected card's Moon and planet detail use their providers' `::new()`. The eclipse and planet-event lists cover 1990–2060, so the eclipse of 585 BC is not listed. Engine changes, one line each (`.with_policy(TierPolicy::WithLabelled)`), once each owner has checked its numbers there |
| The pack prompt on a phone's short stage | open (design, packs) | On a phone the stage between the time bar and the sheet is about 280 px tall. The pack prompt (bottom centre, `packs/packs.css`) and the offline-ready card stack over its lower half. On Charts → Tides they cover the card's Earlier/Later, Day/Week and Save until the prompt is answered (the offline card goes by itself after a few seconds). The prompt could go into the sheet on phones, or fold to one line like the notices do |
| The Sky view's draw budget (10 ms) | open | The median draw is 9.1–12.4 ms at load averages of 5–12.5 on the shared machine, the same with either `opt-level` (ACCURACY section 22). Not measured on a quiet machine. Leaving out the Milky Way and grid layers at low zoom is the first remedy if it needs margin (not done) |
| `sky/view.ts` split by layer | unstarted | About 2 450 lines. A split with no change in behaviour was left out of the polish pass for lack of time |
| `gen_planetdetail.py` `--window` and `--kernel` | unstarted | The other generators take them (`tools/reference/common.py`). This one's fixtures come from several sources at fixed epochs (Horizons, `jup365.bsp`, NASA's catalogues), so the options need a design, not a pass-through |
| Further module-size levers | noted | In cargo's output, serde's code is 738 KB of the 2.38 MB of code, and the sorts' repeated copies are about 115 KB (ACCURACY section 22). One serialiser would change the wire contract; a shared sort would change how the engines order their events. The module is 450 KB raw and 98 KB gzipped inside its budget |
| Behaviour for the owner to confirm | open (owner) | The rise and set cards go blank while time plays faster than a month a second. The phone's time bar is two lines at a fixed height |
| The Learn view while the time bar is dragged | open | Learn does not use the time, yet a drag costs 14–19 ms of script a frame there (100–117 ms frames in headless Chrome), against 1.4 ms on About. Main at 1bff16e measures the same, so the cost predates this pass. Not investigated (ui-check's scrub block reports it) |
