...
| The 10 ms draw budget on a quiet machine | unstarted | measured only under a load average of about 28 on 8 cores: the layers add about 1.5 ms of JavaScript to a frame (5.55 → 7.08 ms, ACCURACY 20); the absolute browser figure (9.7 → 11.2 ms at the median under that load) needs a quiet machine to confirm |
