...
| item | status | notes |
|---|---|---|
| Tide predictions engine and the `tides-us` pack | completed (engine) | `skyfix-tides` + `skyfix-wasm::tides`: NOAA's 3 499 stations, harmonic prediction with Schureman node factors in NOAA's conventions, high and low water with NOAA's tide-table rule, subordinate stations, datums, nearest stations; within 1.36 min and 1.08 cm of NOAA's own predictions at every station tested (`docs/ACCURACY.md` section 14). The interface (Charts → Tides, map layer, Tonight line) is wave 2 |
| Tide pack loading through the pack mechanism | partial | Rust `install_tides_us(payload)` is ready for the `packs::install` dispatcher; until `packs.rs` is on main the module exports a temporary `load_pack_tides_us(bytes)` and the TypeScript engines an optional `loadTidesPack(bytes)`, to be retired once `load_pack` routes `tides-us` |
