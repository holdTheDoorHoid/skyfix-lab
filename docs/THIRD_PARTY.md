...
| Processing | `tools/limb/build.py` (deterministic): heights interpolated bilinearly from the grid at the nodes of a ring around the mean limb (5 760 axis angles × 384 distances from the mean limb, 1/16° apart, ±12°; CONVENTIONS 15.6), rounded to 5 m (at most 2.5 m, 0.0014″), coded as byte residuals of a planar predictor (145 two-byte escapes) and wrapped in the common pack header; the build decodes the result and refuses it unless it matches exactly. The payload format is in EXPLORER_API "Expansion programme P12 — the lunar limb". |
