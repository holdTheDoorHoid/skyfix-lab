...
//! tests pin the results **bit for bit** to `frames::apparent_radec_of_date`, the
//! function the provider called directly before the frame existed.
...
fn the_frame_reproduces_the_unbatched_chain_bit_for_bit() {
    for jd_utc in instants() {
        ...
        for s in catalog::navigational_stars() {
            let direct = apparent_radec_of_date(
                s.ra_j2000_deg,
                s.dec_j2000_deg,
    ...
    for (i, &jd_utc) in jds.iter().enumerate() {
        ...
        for s in catalog::navigational_stars() {
            for &t in &[jd_utc, other, jd_utc] {
                let tt = jd_tt(t);
                let want = apparent_radec_of_date(
                    s.ra_j2000_deg,
                    s.dec_j2000_deg,
                    s.pm_ra_cosdec_mas_per_year,
                    s.pm_dec_mas_per_year,
