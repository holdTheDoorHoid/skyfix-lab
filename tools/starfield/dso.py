...
V_TOL = 1.0
#: The engine's words for each type (dso.rs `DsoType::noun`).
NOUNS = {"oc": "open cluster", "gc": "globular cluster", "pn": "planetary nebula",
         "en": "emission nebula", "rn": "reflection nebula", "snr": "supernova remnant",
         "cn": "star cluster with nebula", "sg": "spiral galaxy", "eg": "elliptical galaxy",
         "lg": "lenticular galaxy", "ig": "irregular galaxy", "ds": "double star",
         "as": "asterism", "sc": "star cloud"}
...
def main() -> int:
    ...
    # ---- Write outputs ----
    from .bsc import CONSTELLATIONS
    const_names = {a: n for a, n in CONSTELLATIONS}
    dropped = 0
    with open(OUT, "w", encoding="utf-8") as f:
        # One line of header: the provenance is in deepsky_manifest.json and
        # docs/THIRD_PARTY.md ("Deep sky"); every byte here ships in the WASM module.
        f.write("# tools/starfield/dso.py; id|type|ra|dec|V|maj|min|name|description|cross_ids\n")
        for oid, typ, ra, dec, v, maj, mnr, name, desc, cross in rows:
            # A description that only says "A <type> in <constellation>" is what the
            # engine writes when there is none (dso.rs), so it is not shipped twice.
            m = re.fullmatch(r"An? ([a-z ]+) in (.+)", desc)
            if m and m.group(1) == NOUNS[typ] and m.group(2) in const_names.values():
                desc = ""
                dropped += 1
    ...
    summary = {
        ...
        "descriptions_left_to_the_engine": dropped,
        "notes": notes,
