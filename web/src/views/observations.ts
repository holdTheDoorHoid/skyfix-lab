...
import { DEMOS, findVariant } from '../demos.js';
import { MockApi, emptySession } from '../api/mock.js';
...
export function renderObservations(store: Store, root: HTMLElement): void {
  ...
  for (const demo of DEMOS) {
    const group = h('optgroup', { label: demo.title } as never) as HTMLOptGroupElement;
    for (const variant of demo.variants) {
      group.appendChild(h('option', { value: variant.id }, variant.label));
    }
    demoSelect.appendChild(group);
  }
  const demoExpect = h('p', { class: 'note' }, 'Each demo is a simulated session. Its truth is shown only on the Simulator view.');
  demoSelect.addEventListener('change', () => {
    const found = findVariant(demoSelect.value);
    demoExpect.textContent = found
      ? `${found.demo.why} What to look for: ${found.variant.expect}`
      : 'Each demo is a simulated session. Its truth is shown only on the Simulator view.';
  });
  ...
  const loadDemo = button('Load demo', () => {
    ...
    void loadScenario(store, found.variant.id);
  });
  ...
  root.appendChild(
    panel(
      'Packaged demos',
      note(
        'The six demos from the brief. Loading one replaces the session below with a freshly simulated one.',
      ),
...
/**
 * Run a demo scenario through the API. If the real adapter cannot simulate yet, fall
 * back to the mock generator and say so rather than showing nothing.
 */
export async function loadScenario(store: Store, variantId: string): Promise<void> {
  const found = findVariant(variantId);
  if (!found) return;
  try {
    const output = await store.api.simulate(found.variant.scenario);
    store.replaceSession(output.session, { demo: variantId });
    store.set({
      simulation: {
        scenario: found.variant.scenario,
    ...
    store.notice('info', `Loaded ${found.demo.title} — ${found.variant.label}.`);
  } catch (error) {
    const output = await new MockApi().simulate(found.variant.scenario);
    store.replaceSession(output.session, { demo: variantId });
    store.set({
      simulation: {
        scenario: found.variant.scenario,
    ...
    store.notice(
      'caution',
      `The simulator is not built into this WebAssembly package yet (${String(error)}). This demo was generated by the browser's mock generator instead.`,
    );
  }
}
