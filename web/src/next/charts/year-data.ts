 ...
 * OWNER: charts agent.
 *
 * One `day_events_batch` call for the whole year (365 or 366 local-midnight windows),
 * one `moon_phases`, one `seasons`. Everything else is arranging the answers on a clock
 * face: local wall-clock hours, so the clock changes show as the jumps they are.
 */
...
import { clockChangeIn, daysOfYear, wallHours, type ClockChange, type LocalDay } from './windows.js';
...
export function computeYear(engine: ExplorerEngine, input: YearInput): YearData {
  ...
  return {
    ...
    shortest,
    errors,
    timing: { engineMs, batchMs, totalMs: now() - t0 },
  };
}
