...
export function placeSearch(ctx: Ctx, options: SearchOptions): { el: HTMLElement; destroy(): void } {
    ...
    list.replaceChildren(
      ...items.map((item, i) => {
        ...
        const li = h(
          ...
          item.kind === 'message' ? icon('info') : icon(item.kind === 'place' ? 'pin' : 'target'),
          h(
            'span',
            { class: 'sf-search__text' },
            h('span', { class: 'sf-search__name' }, item.label),
            item.kind === 'place' && item.where ? h('span', { class: 'sf-search__where' }, item.where) : null,
          ),
        );
        if (selectable) {
  ...
  const run = (): void => {
    ...
    active = items.findIndex((i) => i.kind !== 'message');
    render();
  };
  ...
  const onKey = (e: KeyboardEvent): void => {
    const selectable = items.map((it, i) => (it.kind === 'message' ? -1 : i)).filter((i) => i >= 0);
    switch (e.key) {
      case 'ArrowDown':
      case 'ArrowUp': {
        if (list.hidden) {
