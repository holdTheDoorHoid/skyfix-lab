...
export interface ExplorerWasmExports {
  ...
  /** TEMPORARY until `load_pack` (packs agent) is on main. */
  load_pack_tides_us?(bytes: Uint8Array): unknown;
  // --- end tides
  version?(): string;
}
...
export class WasmEngine implements ExplorerEngine, AlmanacEngine, EclipseEngine, PlanetEventsEngine {
  ...
  // `pack_not_loaded: …` until the tides-us pack is installed.
  // ---------------------------------------------------------------------------------
  ...
  /** TEMPORARY until `loadPack` is on main: install a tides-us file (`load_pack_tides_us`). */
  loadTidesPack(bytes: Uint8Array): TidesPackInfo {
    const fn = this.tideFn('load_pack_tides_us');
    return this.call('load_pack_tides_us', () => fn.call(this.x, bytes));
  }
  // --- end tides
}
